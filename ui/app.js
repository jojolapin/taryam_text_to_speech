...
/* =======================================================================
   TextChunker - splits text at natural boundaries, preserves offsets
======================================================================= */
const TextChunker = {
...
/* =======================================================================
   PiperReader - playback pipeline (session-guarded, prefetched)
======================================================================= */
class PiperReader {
  constructor() {
  ...
  _transition(newState, details = {}) {
    ...
    this.state = newState;
    if (prev !== newState) Logger.info(`state: ${prev} -> ${newState}${details.reason ? ' (' + details.reason + ')' : ''}`);
  ...
  async start(text, opts = {}, fromPosition = 0) {
    if (!text || !text.trim()) { this._emit('error', { message: I18N.t('toast.noText') }); return false; }
    if (!opts.voice) { this._emit('error', { message: I18N.t('toast.noVoice') }); return false; }
    ...
    this.chunks = TextChunker.chunk(text, 450);
    if (!this.chunks.length) { this._emit('error', { message: 'No chunks.' }); return false; }
    this.chunkIdx = TextChunker.findChunkAtPosition(this.chunks, Math.max(0, Math.min(fromPosition, text.length)));
  ...
  _ensureChunkLoaded(idx, sid) {
    ...
    const promise = BridgeAPI.synthesize(chunk.text, this.opts.voice, 1.0 / this.opts.rate, this.opts.volume, this.opts.textFormat || 'plain');
    ...
    promise.then(({ wavB64 }) => {
      ...
      Logger.debug(`chunk ${idx + 1} loaded (${(blob.size / 1024).toFixed(1)} KB)`);
      this._resolveChunkWaiters(idx, null);
    }).catch(e => {
      if (this.sessionId !== sid) return;
      Logger.error(`chunk ${idx + 1} failed: ${e.message}`);
  ...
  _onChunkEnded() {
    ...
    const sid = this.sessionId;
    const entry = this.audioCache.get(this.chunkIdx);
    if (entry) { URL.revokeObjectURL(entry.url); this.audioCache.delete(this.chunkIdx); }
    this.chunkIdx += 1;
    if (this.chunkIdx >= this.chunks.length) { this._finish(); return; }
    this._ensureChunkLoaded(this.chunkIdx + 1, sid);
  ...
  _hardStop(reason, silent = false) {
    if (!silent) Logger.debug('hard stop: ' + reason);
    this.sessionId += 1;
    ...
    this.audioCache.forEach(e => { try { URL.revokeObjectURL(e.url); } catch {} });
