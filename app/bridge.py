...
class _Runnable(QRunnable):
    ...
    def run(self):  # noqa: D401
        try:
            ...
            log.debug("Background operation cancelled")
...
class Bridge(QObject):
    ...
    def _effective_format(self, fmt: str, text: str) -> str:
        ...
        if (fmt or "").startswith("document:"):
            explicit = fmt.split(":", 1)[1]
            explicit = {"off": "plain", "on": "markdown"}.get(explicit, explicit)
            return explicit if explicit in {"plain", "markdown", "html"} else text_normalize.detect_format("<memory>", text=text)
    ...
    def batch_export(self, paragraphs_json: str, voice_id: str, fmt: str,
        ...
        outputs = [Path(folder) / _safe_filename(f"{prefix}-{i:03d}", ext) for i in range(1, len(paragraphs)+1)]
        if any(out.exists() for out in outputs):
            self.exportError.emit(request_id, "An output file already exists. Choose an empty folder or a different bookmark title.")
            return
        token = CancelToken()
        self._cancels[request_id] = token
